...
#define MAX_SEND_MOR_RADIO_TIME	5 	/* 5 MIN */
...
static volatile int g_db_reload							= 0;
...
static volatile UINT32 g_last_websend_time = 0;
...
static int send_web_noti( UINT16 web_msg, void * ptrdata );

static int load_db( void );
static int check_rtu_net_chg( void );
...
static int send_data_apc_ups( UINT16 inter_msg, void * ptrdata );
...
static int recv_onoff_req( UINT16 inter_msg, void * ptrdata )
{
	...
	//print_buf( sizeof( onoff_data ), ptrmsg->ptrbudy );

	//printf("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk ctltype:%d, ctlcmd:%d \r\n", 
	//		onoff_data.ctl_base.ectltype,
	//		onoff_data.ctl_base.ectlcmd);
	/* DO OFF 요청 */
...
static int recv_allim_web_req( UINT16 inter_msg, void * ptrdata )
{
	...
	if ( allim_msg.allim_msg == WEB_RTU_ADD_CLIENT  )
	{
		print_dbg( DBG_INFO,1, "Control Recv Add Web Client From Websock ");
		g_last_websend_time = 0;
	}
...
static int send_data_apc_ups( UINT16 inter_msg, void * ptrdata )
{
	...
	ret = recv_ctrl_data_apc_ups( inter_msg, ptrdata );
...
static int send_network_chang( int msg )
{
	...
	{
		send_data_apc_ups(cmd, (void *)&inter_msg);
	}
...
static int send_ai_data(int clientid, UINT16 cmd, int send_op )
{
	...
	for( i = 0; i < MAX_APC_UPS_DATA_CNT ; i++ )
	{
		...
		{
			if ( g_apc_ups_ai_val[ i ].snsr_id != 0 && g_apc_ups_ai_val[ i ].snsr_sub_type != 0 && 
				 (( g_apc_ups_ai_val[ i ].last_val >=0 || sts ==  ESNSRCNNST_ERROR ))) 
			{
				sts = g_apc_ups_sr_timeout;
...
static int send_web_noti( UINT16 web_msg, void * ptrdata )
{
	...
	if ( ptrdata != NULL )
	{
		memcpy( &web_noti, ptrdata, sizeof( web_noti ));
	}

	web_noti.site_id= get_my_siteid_shm();
	web_noti.web_msg = web_msg;

	/* intr msg */
	cmd = EFT_ALLIM_WEB_NOTI;
	inter_msg.client_id	= 0;
	inter_msg.msg 		= cmd;
	inter_msg.ptrbudy 	= &web_noti;

	print_dbg(DBG_INFO,1,"########## Send to WebSocket that Update DB Ctrol");
	send_data_transfer( cmd, (void *)&inter_msg , SEND_WEB);
...
static int check_cur_data( UINT32 cur_time )
{
	...
	inter_time = cur_time - g_last_websend_time;
...
static int internal_rtu_fclt_idle( void )
{
...
static int insert_rtu_fclt( int fclt_code , char * ptrorgip, char * ptrorgport )
{
	int i;
	int e_pos = 0;
	...
	/* UPS FCLT 등록 */
	g_rtu_fclt_info[ e_pos ].fcltid = get_my_rtu_fcltid_shm()  ;
	g_rtu_fclt_info[ e_pos ].fcltcode = fclt_code;

	if ( ptrorgip != NULL )
	{
		memcpy( g_rtu_fclt_info[ e_pos ].szip , ptrorgip, strlen( ptrorgip ) );
	}
	if ( ptrorgport != NULL )
	{
		g_rtu_fclt_info[ e_pos ].port = atoi( ptrorgport );
	}
	
	print_dbg( DBG_SAVE,1, "NEW RTU FCLT Insert Index:%d, FCLTID:%d, FcltCode :%d, IP:%s, Port:%d", e_pos,g_rtu_fclt_info[ e_pos ].fcltid, 
							g_rtu_fclt_info[ e_pos ].fcltcode, g_rtu_fclt_info[ e_pos ].szip, g_rtu_fclt_info[ e_pos ].port);
	...
	for( i = 0 ; i < ( MAX_GPIO_CNT * 2 ) ; i++ )
	{
		...
		else if ( temp_di[ i ].snsr_sub_type == ESNSRSUB_DI_UPS_LOW_INPUT )
		{
			...
			//printf("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk2\r\n");
				...
				print_dbg(DBG_SAVE,1, "UPS IP:%s",  temp_di[ i ].szip, MAX_IP_SIZE );
				print_dbg( DBG_SAVE,1, "UPS PORT:%s", temp_di[ i ].szport, MAX_PORT_SIZE );
				insert_rtu_fclt( EFCLT_UPS , temp_di[ i ].szip, temp_di[ i ].szport );
			}
	...
	for( i = 0 ; i < MAX_GRP_RSLT_AI ;i++ )
	{
				  ...
				  temp[ i ].snsr_sub_type == ESNSRSUB_AI_UPS_BATTERY_REMAIN_TIME )	
		{
			...
			if ( ups_cnt < MAX_APC_UPS_DATA_CNT )
			{
				...
				ups_cnt++;
				print_dbg(DBG_SAVE,1, "UPS IP:%s",  temp[ i ].szip, MAX_IP_SIZE );
				print_dbg( DBG_SAVE,1, "UPS PORT:%s", temp[ i ].szport, MAX_PORT_SIZE );
...
static int check_mcu_event( void )
{
	...
	for( i = 0 ; i < MAX_GPIO_CNT ; i++ )
	{
		...
		if ( (last_sts != last_val ) || ( send_alarm > 0 ) )
		{
...
static int check_temp_event( void )
{
	...
	for( i = 0 ; i < MAX_TEMP_CNT ; i++ )
	{
        ...
        if ( g_temp_val[ i ].snsr_id > 0 &&  g_temp_val[ i ].snsr_sub_type > 0 )
        {
            ...
            if ( last_sts != range || send_alarm > 0 )
            {
...
static int check_ground_event( void )
{
	...
	if ( last_val != last_sts || send_alarm > 0 )
	{
        if ( g_ground_val.snsr_id > 0 &&  g_ground_val.snsr_sub_type > 0 )
        {
			fault_cnt = 0;
...
int set_apc_ups_trans_sts_ctrl(int val )
{
	...
	int i;
	int snsr_id =0;
	static volatile int err_cnt =0;
		
	for( i = 0 ; i < MAX_APC_UPS_DATA_CNT ; i++ )
	{
		sub_type = g_apc_ups_di_val[ i ].snsr_sub_type;
		snsr_id  = g_apc_ups_di_val[ i ].snsr_id ;

		if ( sub_type  == ESNSRSUB_DI_UPS_LOW_INPUT ) 
		{
			break;
		}
	}
	...
	if( g_apc_ups_sr_timeout != val )
	{
		...
		send_sns_netsts( val, ESNSRTYP_DI, ESNSRSUB_DI_UPS_LOW_INPUT, snsr_id);
        flag = 1;
    }
