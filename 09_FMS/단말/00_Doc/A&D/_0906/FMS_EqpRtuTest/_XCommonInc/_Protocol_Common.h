...
typedef struct _FMS_EVT_FaultDI // 장애 발생 정보
{
	FMS_ElementFclt     tFcltElem;		// 설비까지 구분
	FMS_TransDBData     tDbTans;		// RTU-SVR 간 DB 트랜젝션 확인을 위한 정보
...
typedef struct _FCLT_EVT_FaultDI : public FMS_EVT_FaultDI // 장애 발생 정보
{
...
typedef struct _FCLT_EVT_DBDataUpdate
{
	FMS_ElementFclt     tFcltElem;		// 설비까지 구분
	// -------------------------
	int					iNotifyFlag;	// 업데이트 발생한 테이블 정보(OP, SVR -> RTU) : enumDBEditInfo 참조
