...
os.chdir(script_dir)
MODEL_DIR = os.path.abspath(os.path.join(script_dir, "../00_Models"))
IMG_DIR = os.path.abspath(os.path.join(script_dir, "../00_Sample_video/input"))
...
def app():
    with gr.Blocks():
        with gr.Row():
            with gr.Column():
                ...
                model_id = gr.Dropdown(
                    label="Model",
                    choices=[
                        os.path.join(MODEL_DIR, "yolov12n.pt"),  # 기본값을 경량 모델로 변경
                        os.path.join(MODEL_DIR, "yolov12s.pt"),
                        os.path.join(MODEL_DIR, "yolov12m.pt"),
                        os.path.join(MODEL_DIR, "yolov12l.pt"),
                        os.path.join(MODEL_DIR, "yolov12x.pt"),
                    ],
                    value=os.path.join(MODEL_DIR, "yolov12n.pt"),  # 기본값을 경량 모델로 설정
                )
        ...
        gr.Examples(
            examples=[
                [
                    os.path.join(IMG_DIR, "person.png"),
                    os.path.join(MODEL_DIR, "yolov12s.pt"),
                    640,
                    0.25,
                ],
                [
                    os.path.join(IMG_DIR, "car.png"),
                    os.path.join(MODEL_DIR, "yolov12x.pt"),
                    640,
                    0.25,
                ],
