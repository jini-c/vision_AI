...
class KITTILoader:
    def __init__(self, subset="training"):
        ...
        self.base_dir = "../00_Dataset/kitti/"
...
def plot3d(
    ...
    """
    3D 바운딩 박스를 이미지 위에 그리며, 라벨, confidence, 객체 ID를 표시합니다.
    """
    # 3D 바운딩 박스 계산
    location, X = calc_location_(dimensions, proj_matrix, box_2d, alpha, theta_ray)

    # orient = (alpha + theta_ray)으로 회전 각도를 계산
    orient = (alpha + theta_ray) % (2 * np.pi)

    # 2D 바운딩 박스 표시 (Optional)
    ...
    if isinstance(confidence, (list, np.ndarray)):
        if (
            isinstance(confidence, np.ndarray) and confidence.ndim == 0
        ):  # 0차원 numpy 배열 처리
            confidence = float(confidence.item())
        else:
            confidence = float(max(confidence))  # 가장 높은 confidence 값 선택
    elif isinstance(confidence, np.generic):  # numpy scalar 처리
        confidence = float(confidence.item())
    elif isinstance(confidence, (torch.Tensor,)):
        confidence = float(confidence.item())  # 텐서인 경우 float로 변환
    elif isinstance(confidence, float):
        confidence = confidence  # 이미 float 값인 경우 그대로 사용

    # 객체 ID 처리
    if obj_id is not None:
        if isinstance(obj_id, list):  # 리스트인 경우
            obj_id = obj_id[0]  # 첫 번째 요소 추출
        if isinstance(obj_id, torch.Tensor):  # torch.Tensor 처리
            obj_id = int(obj_id.item())
        elif isinstance(obj_id, np.ndarray):  # numpy.ndarray 처리
            obj_id = int(obj_id[0]) if obj_id.ndim > 0 else int(obj_id.item())
        elif isinstance(obj_id, (int, float)):  # 이미 정수 또는 실수인 경우
            obj_id = int(obj_id)

    # 라벨, confidence, 객체 ID 표시
    if label is not None or confidence is not None or obj_id is not None:
        id_text = f"ID: {obj_id}" if obj_id is not None else ""
        confidence_text = (
            f" ({confidence * 100:.1f}%)" if confidence is not None else ""
        )
        text = f"{label}{confidence_text} {id_text}".strip()

        text_size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1)
        text_w, text_h = text_size[0]

        x_min, y_min = int(box_2d[0]), int(box_2d[1])  # 바운딩 박스 좌상단
        # 텍스트 배경 그리기
        cv2.rectangle(
            img,
